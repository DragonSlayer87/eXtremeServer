...
namespace server
{
    ...
    const char *colorname(clientinfo *ci, const char *name)
    {
        ...
        static int cidx = 0;
        cidx = (cidx+1)%3;
        formatstring(cname[cidx], ci->state.aitype == AI_NONE ? "%s\fs\f4(%d)\fr" : "%s\fs\f4(%d)\fr", name, ci->clientnum);
        return cname[cidx];
    }
    ...
    //remod
    VAR(imissiontime, 0, 20000, INT_MAX);
	...
	// allow all weapons to pass flag when hitting target
	VAR(r_allowrifle,0,1,1);
    VAR(r_allowshotgun,0,0,1);
    VAR(r_allowminigun,0,0,1);
    VAR(r_allowrocket,0,0,1);
    VAR(r_allowgrenade,0,0,1);
    VAR(r_allowpistol,0,0,1);
    VAR(r_allowchainsaw,0,0,1);

    void dodamage(clientinfo *target, clientinfo *actor, int damage, int gun, const vec &hitpush = vec(0, 0, 0))
    {
		// goldmod
		// rugbypass event - supports all multiplayer weapons
		bool*canrugby=((bool*)false);
		if(rugbymode&&actor!=target&&isteam(actor->team,target->team)&&m_ctf&&ctfmode.isflagholder(actor))
		{
			switch(gun)
			{
				case GUN_RIFLE:canrugby=((bool*)r_allowrifle);break;
				case GUN_SG:canrugby=((bool*)r_allowshotgun);break;
				case GUN_CG:canrugby=((bool*)r_allowminigun);break;
				case GUN_RL:canrugby=((bool*)r_allowrocket);break;
				case GUN_GL:canrugby=((bool*)r_allowgrenade);break;
				case GUN_PISTOL:canrugby=((bool*)r_allowpistol);break;
				case GUN_FIST:canrugby=((bool*)r_allowchainsaw);break;
				default:canrugby=((bool*)false);//useless :)
			}
			if(canrugby){ctfmode.rugbypass(actor,target);return;}
		}
		// end of rugbymode
        // block damage if hider hits the seeker (only for hide and seek plugin)
        if(m_teammode&&!seekdamage&&(strcmp(target->team,seekteam)!=-1)&&(strcmp(actor->team,hideteam)!=-1))
        {
            remod::onevent(ONSEEKERHIT,"ii",actor->clientnum,target->clientnum);
            return;
        }// hider can't hit the seeker (only in team modes)
        ...
        // no team damage
        if(m_teammode&&!teamdamage&&(strcmp(target->team,actor->team)!=-1)){damage=0;}
        // end of goldmod*
        
        // remod
        if(m_edit && nodamage == 1) return; // no damage if nodamage is enabled (only works in coop edit mode)
        if(!anydamage)return;//return if anydamage is disabled (disable aĺl kinds of damage)
        ...
        {
            // goldmod
            // chainsawhack detection event trigger
            if(gun==GUN_FIST)remod::onevent(ONCHAINSAW,"ii",actor->clientnum,target->clientnum);
            // remod
	...
	// returns 1 if player is alive and 0 if player is not alive (e.g. dead, spectator, not connected;
	void isalive(int *cn)
	{
		clientinfo*ci=(clientinfo*)getinfo((int)*cn);
		if(!ci)return; // return if invalid cn
		intret(ci->state.state==CS_ALIVE?1:0);
	}
	COMMAND(isalive,"i");
    ...
    void ircputs(int *cn, char *msg)
    {
        clientinfo *ci = getinfo(*cn);
        if(!ci || ci->privilege >= PRIV_AUTH || (ci->state.aitype==AI_NONE)) return;
        flushserver(true);
        uchar buf[MAXTRANS];
    ...
    COMMANDN(ircbottalk, ircputs, "is");
        ...
        while((curmsg = p.length()) < p.maxlen) switch(type = checktype(getint(p), ci))
        {
            ...
            case N_TEXT:
            {
                ...
                if(strlen(ftext)>strlen(commandchar) && (strncmp(commandchar, ftext, strlen(commandchar)) == 0))
                {
                    ...
                    remod::oneventi(ONCOMMAND, "is", sender, ftext);
                    //conoutf(ftextchf);
                    break;
                }
				...
				if(mapvote_work&&((strcmp("1",text)==0)||(strcmp("2",text)==0)))
                {
                    remod::onevent(ONVOTEFORMAP,"is",ci->clientnum,text);
                    break;
                }
            ...
            case N_SAYTEAM:
            {
                ...
                if(ci->spy)
                { 
                    const char *message_team=tempformatstring("\fs[\f4SPY\f7] (Team) \f7(\f3%s\f7): \f4%s\f7",ci->name,tx);
                    loopv(clients){if(clients[i]->state.aitype==AI_NONE&&(strcmp(ci->team,clients[i]->team)==0)){sendf(clients[i]->clientnum,1,"ris",N_SERVMSG,message_team);}}
                ...
                loopv(clients)
                {
                    ...
                    if(!cq->spy)sendf(t->clientnum, 1, "riis", N_SAYTEAM, cq->clientnum, text);
                }
            ...
            case N_EDITVAR:
            {
				if(remod::checkflood(ci, N_EDITVAR)){sendservmsg("ioi");break;}
                if(ci->state.ext.ghost) break;
            ...
            case N_CLEARBANS:
            {
                if(ci->privilege || ci->local)
                {
                    ...
                    // goldmod
                    char*str;
                    str=tempformatstring("Player \f3%s(%i) \f7has cleared all temporal bans on \f4server\f7.",ci->name,ci->clientnum);
                    sendservmsgf(str);
