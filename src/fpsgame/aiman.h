...
VAR(enable_bot,0,0,1);
SVAR(ircainame,"IRC");
SVAR(ainame, "bot"); // change bot name
...
namespace aiman
{
	...
	// goldmod
    // modified version of remod-racing's server talk bot
    bool addservai()
    {
    ...
    if(numai >= maxai) return false;
      ...
      clients.add(ci);
      ci->state.lasttimeplayed = lastmillis;
      copystring(ci->name, ircainame, MAXNAMELEN+1);
      copystring(ci->team, team, MAXTEAMLEN+1);
      ci->playermodel = rnd(128);
      ci->ping = 123;
      ci->aireinit = 2;
    ...
    COMMANDN(addserverircbot,addservai,"");

	void deleteai(clientinfo *ci)
	{
        // goldmod
        if(ci&&ci->state.aitype==AI_SERVIRCBOT)return; // return if bot is server irc bot
        // remod
    ...
    void reqdelservai(int cn)
	{
		...
		if(!ci || ci->state.aitype!=AI_SERVIRCBOT) return;
		deleteservai(ci);
	}
	...
	ICOMMAND(delservai, "", (), deletelastservai());
