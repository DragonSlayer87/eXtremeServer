...
// goldmod
// declare variables what can't be changed
void constant(char*name)
{
	ident*id=idents.access(name);
	if(!id)return;
	id->flags&=IDF_READONLY;
	debugcode("inizializing variable %s as constant",name);
}
COMMAND(constant,"s");
...
// goldmod
//useless functions to change values of integers
...
ICOMMAND(cbrt,"f",(float*a),floatret(cbrt(*a)));//goldmod - return cubic root of *a
