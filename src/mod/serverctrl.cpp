...
{
    string txt;
    formatstring(txt, "eXtremeServ %s %s (build: %s %s) %s/%s", REMOD_CODENAME, REMOD_VERSION, __DATE__, __TIME__, REMOD_SYSTEM, REMOD_ARCH);
    result(txt);
}
...
{
    clientinfo *ci = (clientinfo *)getinfo(*cn);
    if(!ci || ci->state.state == CS_SPECTATOR||ci->spy) return;
...
void disc(int cn)
{
	clientinfo*target=getinfo(cn);
	if(!target||target->spy)return;
	disconnect_client(cn,DISC_FORCEDISCONNECT);
}
 ...
 * @arg1 client number
 */
ICOMMAND(disconnect,"i",(int cn),disc(cn));
